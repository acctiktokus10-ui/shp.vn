  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/shorten", {
        ...
        body: JSON.stringify({ url: url.trim(), subId: "" }),
      });
        ...
        <div className="card">
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            ...
            <h1 style={{
              ...
              marginBottom: 4,
            }}>🎀 Test Link Nè Hihi 🎀</h1>
            <p style={{ color: "#ff8fab", fontSize: 12, fontWeight: 600 }}>
              ✨ Dán link Shopee → nhận short link siêu cute~ ✨
            </p>
          </div>
          ...
          {error && (
            <div style={{
              ...
              borderRadius: 12, color: "#e53935", fontSize: 13, fontWeight: 600,
            }}>😿 {error}</div>
          )}
          ...
          {result && (
            <div style={{
              ...
              border: "2px solid #ffd6e0", borderRadius: 16,
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: "#ff6b9d", marginBottom: 4, textTransform: "uppercase", letterSpacing: 1 }}>🎀 Short link</div>
                  <div style={{ fontSize: 13, color: "#333", fontWeight: 700, wordBreak: "break-all" }}>{result.shortUrl}</div>
                </div>
                <button
                  className="copy-btn"
                  style={{ background: copied ? "linear-gradient(135deg,#43e97b,#38f9d7)" : "linear-gradient(135deg,#ff6b9d,#ff4d6d)", color: "white" }}
